...
typedef enum SLFIP {
  ...
  SetGeneric                                        = 0x85, // Deprecated - see SetSystemValue.
...
typedef enum {
  SL_RESET_PARAMETERS      = 0, //!< Reset all parameters (including network, serial port, cameras)
  SL_RESET_VIDEO           = 1, //!< Reset the video decoder
  SL_RESET_BOARD           = 2, //!< Restart the board
  ...
  SL_RESET_PARAMETERS_SOFT = 6, //!< Resets "soft" parameters (not serial port, network, camera type)
  SL_RESET_LAST               
} SLA_RESET_TYPE;
...
s32 SLFIPSetCoordinateReportingPeriod(SLPacketType buffer, u8 framePeriod, u16 reportFlags, u8 cameraIndex);

s32 SLFIPSetTrackingParameters(SLPacketType buffer, u8 objectSize, u8 mode, u8 noiseMode, u8 maxMisses, u8 reserved1 = 0, u16 nearVal=0, u8 acqAssist = 0, u8 intellAssist = 0, u8 objectHeight =0);
s32 SLFIPSetTrackingParameters(SLPacketType buffer, u8 objectSize, u8 mode, u8 noiseMode, u8 maxMisses, u8 reserved1, u16 nearVal, u8 acqAssist, u8 intellAssist, u8 objectHeight, u8 cameraIdx);
...
s32 SLFIPSetStabilizationBias(SLPacketType buffer, s8 colBias, s8 rowBias, bool autoBias, u8 updateRate, u8 cameraIndex);
...
s32 SLFIPSetDisplayParameters(SLPacketType buffer, u16 rotationDegrees, u16 rotationLimit, u8 decayRate, u8 falseColor, u8 displayZoom, u8 zoomToTrack, s16 panCol, s16 tiltRow);
...
s32 SLFIPSetDisplayParameters(SLPacketType buffer, u16 rotationDegrees, u16 rotationLimit, u8 decayRate, u8 falseColor, u8 displayZoom, u8 zoomToTrack, s16 panCol, s16 tiltRow, u8 idx);
...
typedef struct {
  ...
  u8  DebugFiltering; // Debug flags to help user understand filtering based on size,velocity, and acceleration.
...
s32 SLFIPSetDetectionRegionOfInterestParameters(SLPacketType buffer, SLFIPMotDetRoiParams params, u8 cameraIdx);
...
s32 SLFIPGetADCParameters(SLPacketType buffer, u8 cameraIndex);
...
typedef enum {
  SL_RECORD_H264             = 0x01,
  //SL_RECORD_RESERVED         = 0x02,
  //SL_RECORD_RESERVED         = 0x04, 
  //SL_RECORD_RESERVED         = 0x08,
  //SL_RECORD_RESERVED         = 0x10,
  //SL_RECORD_RESERVED         = 0x20,
  SL_RECORD_KB_OR_FRAMES     = 0x40, //0 = video length in kbytes, 1 = video length in frames
  SL_RECORD_SCAN_AUTONUMBERING = 0x80
} SL_RECORD_TYPE;
...
typedef struct
{
  s16 row;
  s16 col;
  u8  fontId;       // set bit7 == 1 to add black shadow, set bit6 == 1 to add a black outline @see SLFONT_TYPE
  u8  fontWidth;
...
typedef struct
{
  s16 row;
  s16 col;
  u8 scale5;            // 32 == x1 scale
  u8 fontId;            // set bit7 == 1 to add black shadow, set bit6 == 1 to add a black outline @see SLFONT_TYPE
  u8 klvField;          // which KLV field to draw
  u8 klvFormat;         // which format to draw use (specific to the klv field)
  u16 len;
  char text[MAX_TEXT_LEN];
} SL_KLV_DRAW_PARAMS;
...
enum {
  ...
  SL_DRAW_FLAGS_SAVE              = 0x40,   //!< Save object to parameter file
...
typedef enum {
  ...
  SLFIP_DO_KLV_FIELD         = 8,  // KLV Field
  SLFIP_DO_STD_TYPE_LAST     = 9,
  ...
  SLFIP_DO_CUSTOM_6,
  SLFIP_DO_CUSTOM_7,
  SLFIP_DO_MAX               = 255, // Set to 255 per the IDD
...
s32 SLFIPDrawObject(SLPacketType buffer, u8 objId, u8 action, u8 sourceCoords, u8 moveWithCamera, u8 staticCoordinates, 
                    u8 upperLeftOrigin, u8 type, void *params, u8 backgroundColor, u8 foregroundColor, u8 save = 0);
...
typedef enum {
  ...
  SL_BOARD_TYPE_SLA3000_REV_B = 12, 
  SL_BOARD_TYPE_WIN3000_REV_B = 13,
  SL_BOARD_TYPE_SLA3000_REV_C = 14,
  SL_BOARD_TYPE_WIN3000_REV_C = 15,
  SL_BOARD_TYPE_Last
} SL_BOARD_TYPE;

// Uncomment below lines to build software for 2.23 or earlier. 
//#define SL_BOARD_TYPE_SLA3000 SL_BOARD_TYPE_SLA3000_REV_B
//#define SL_BOARD_TYPE_WIN3000 SL_BOARD_TYPE_WIN3000_REV_B 

SLINLINE static bool Is3000_HW(SL_BOARD_TYPE type) {
  return type == SL_BOARD_TYPE_SLA3000_REV_B || type == SL_BOARD_TYPE_SLA3000_REV_C;
}
SLINLINE static bool Is3000_WIN(SL_BOARD_TYPE type) {
  return type == SL_BOARD_TYPE_WIN3000_REV_B || type == SL_BOARD_TYPE_WIN3000_REV_C;
}
SLINLINE static bool Is3000_ANY(SL_BOARD_TYPE type) {
  return Is3000_HW(type) || Is3000_WIN(type);
}

SLINLINE static bool Is1500_ANY(SL_BOARD_TYPE type) {
  return type == SL_BOARD_TYPE_SLA1500 || type == SL_BOARD_TYPE_SLA1501 || type == SL_BOARD_TYPE_WIN1500;
}

SLINLINE static bool IsWinEmulation_ANY(SL_BOARD_TYPE type) {
  return type == SL_BOARD_TYPE_WIN1500 || type == SL_BOARD_TYPE_WIN3000_REV_C ||
    type == SL_BOARD_TYPE_WIN3000_REV_B;
}

// Macro to be used in switch (boardType).
#define CASE_FOR_ANY_SL_BOARD_3000 \
    SL_BOARD_TYPE_SLA3000_REV_B:        \
    case SL_BOARD_TYPE_SLA3000_REV_C:   \
    case SL_BOARD_TYPE_WIN3000_REV_B:   \
    case SL_BOARD_TYPE_WIN3000_REV_C    \
...
s32 SLFIPTrackingPosition(SLPacketType buffer, 
                          ...
                          s16 offsetRow, s16 offsetCol, u16 rotation, u8 idx, u8 reserved, u16 flags=0, u32 frameId=0xFFFFFFFF, u64 timestamp=0);
...
typedef enum {  
  ...
  SL_ENET_VID_SIZE_320x240      = 8,  // 320x240
...
typedef enum {
  SL_DISP_TR = 0,	//!< TOP RIGHT (Right if all or mti tracks)
  SL_DISP_BR = 1,	//!< BOTTOM RIGHT (Bottom if all or mti tracks)
  SL_DISP_BL = 2,	//!< BOTTOM LEFT (Left if all or mti tracks)
  SL_DISP_TL = 3, //!< TOP LEFT (Top if all or mti tracks)
...
typedef enum {
  ...
  PIP_SCALE_ALL_TRACK = 7,     //!< All tracks - size based on output video size
  PIP_SCALE_MTI_TRACK_5  = 8,  //!< Up to 5 mti tracks - size based on output video size
  PIP_SCALE_MTI_TRACK_10 = 9,  //!< Up to 10 mti tracks - size based on output video size
  PIP_SCALE_MTI_TRACK_8  = 10, //!< Up to 8 mti tracks in the corners - size based on output video size
} SL_PIP_SCALE;
...
s32 SLFIPSetDetectionParams(SLPacketType buffer, u8 mode, int debugCode, u8 threshold, 
                         ...
                         u8 modeUpperByte = 0, u16 minTemperature = 0, u16 maxTemperature = 0);
s32 SLFIPSetDetectionParams(SLPacketType buffer, u8 mode, int debugCode, u8 threshold,
  u8 manualThreshold, u8 manualWatchFrames, u8 suspScore, u8 frameStep,
  u8 modeUpperByte, u16 minTemperature, u16 maxTemperature, u8 cameraIdx);
...
s32 SLFIPSetBlendParams(SLPacketType buffer, u8 mode, bool absOff, s8 vertical, s8 horizontal,
                        ...
                        u8 fixedIdx, u8 usePresetAlign = 0, u8 presetAlignIndex = 0, u8 zoomMul = 0, u8 hzoom = 0);
...
typedef enum {
  ...
  SL_PORT_SERIAL5                     = 8, //!< Reserved
  SL_PORT_SERIAL6                     = 9, //!< Reserved
  SL_PORT_SERIAL7                     = 10,//!< Reserved
  SL_PORT_RESERVED_PASS_THROUGH_LOG   = 11,//!< Reserved
  ...
  SL_PORT_PASS_THROUGH_CMD            = 14,//!< Used internally for Pass Through Command.
  SL_PORT_END
} SLPortID;
...
typedef enum {
  ...
  SL_PF_FIP2        = (1<<SL_PORT_FIP2),
  SL_PF_PASS_THROUGH_CMD = (1<<SL_PORT_PASS_THROUGH_CMD),
...
//!< Describes different functionality for a port
typedef enum {
...
enum {
  SLFIP_ACQPARAM_FLG_INPUT_DATA_MASK= 0x0007,   // Bit[0:2] Input Data Mode (not flags) (platform dependent)
  SLFIP_ACQPARAM_FLG_INPUT_DATA_GRAY  = 0x0,    //  0 = generic gray scale.
  SLFIP_ACQPARAM_FLG_INPUT_DATA_COLOR = 0x1,    //  1 = generic color.
  SLFIP_ACQPARAM_FLG_INPUT_DATA_G8IN16= 0x2,    //  2 = Gray 8bit video but input as 16 bit.
  SLFIP_ACQPARAM_FLG_INPUT_DATA_BAYER = 0x3,    //  3 = Bayer.
  ...
  SLFIP_ACQPARAM_FLG_INIT_CODE_MASK = 0x03C0,   // Bit[9:6] Camera Initialization Routine
  SLFIP_ACQPARAM_FLG_INIT_NONE      = 0x0000,   // Bit[9:6] No Camera Init
  ...
  SLFIP_ACQPARAM_FLG_INIT_TAU       = 0x0006,   // Bit[9:6] Init TAU direct connect
  SLFIP_ACQPARAM_FLG_INIT_ZAFIRO    = 0x0007,   // Bit[9:6] Init DRS Zafiro
  SLFIP_ACQPARAM_FLG_INIT_AR0134CS  = 0x0008,   // Bit[9:6] Init Airborne Innovations Aptina Sensor
  SLFIP_ACQPARAM_FLG_INIT_CAMERALINK= 0x0009,   // Bit[9:6] Init Cameralink
  SLFIP_ACQPARAM_FLG_INIT_BOSON     = 0x000A,   // Bit[9:6] Init FLIR Boson
  ...
  SLFIP_ACQPARAM_FLG_HW_CROP        = 0x0C00,   // Bit[11:10] 11b = use hardware crop (3000 only)
                                                // Bit[11:10] 00b = none of the above.
...
s32 SLFIPSetAcqParams( SLPacketType buffer, u8 cameraIndex, u8 cameraType, 
                        ...
                        u16 validRow = 0, u16 validCol = 0, u16 validHigh = 0, u16 validWide = 0, const char *optArgs = 0,
                        u16 bigHigh = 0, u16 bigWide = 0, u16 bitVertFrontPorch = 0, u16 bitHorzFrontPorch = 0);
...
enum {
  ...
  SLFIP_METADATA_SENSORFIELDOFVIEW = 18,  // HFOV & VFOV
  SLFIP_METADATA_SENSOR_ALT_HAE = 19, 
  SLFIP_METADATA_FRAME_CENTER_ALT_HAE = 20,
  SLFIP_METADATA_TARGET_INFO = 21,        //!< information for the target calc. 
  SLFIP_METADATA_STATIC_LAST
};
...
s32 SLFIPDoSnapShot(SLPacketType buffer, u8 frameStep, u8 numFrames, const char * fileName, u8 snapAllCamerasMask=0, u8 resumeCount=false);
...
typedef struct {
  ...
  u8 reserved;          //!< Reserved - prior to 2.24 used to store training state
  u8 mti;               // Mti payload
} SLFIPTrackRes;
...
s32 SLFIPDigiVidParseParams(SLPacketType buffer, const u8 camIdx, const bool enable, const bool startAt0, const bool oneFipPerImage, const bool usePixelValue, const bool parseAfterAcquired = 0);
